...
export default function App() {
  ...
  const orderedComparisonSegments = useMemo(
    () =>
      SEGMENTS.filter((seg) => comparisonSegments.includes(seg.id)).map(
        (seg) => seg.id,
      ),
    [comparisonSegments],
  );
  ...
  function handleComparisonSegmentsChange(segmentIds) {
    const ordered = SEGMENTS.filter((seg) => segmentIds.includes(seg.id)).map(
      (seg) => seg.id,
    );
              ...
              {selectedSegment ? (
                <>
                  ...
                  <CategoryFilter
                    ...
                    confidence={
                      currentSegmentState?.insights?.confidence ?? null
                    }
                  />
